...
fn main() {
    ...
    let image_height = calcs::calculate_image_height(image_width, aspect_ratio);
    ...
    for j in 0u32..image_height
    {
        for i in 0u32..image_width
        {
            ...
            let ray_direction = pixel_center + camera_center*(-1.);
            
            let r = Ray {orig : camera_center, direc : ray_direction};
...
fn ray_color( R : ray::Ray) -> vec3::Vec3
{
    ...
    return (vec3::Vec3 {x : 1., y : 1., z : 1.}) * (1. - a) + (vec3::Vec3 {x : 0.7, y : 0.5, z : 1.}) * a;
}
